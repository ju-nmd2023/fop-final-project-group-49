    ...
    document.addEventListener("keydown", (event) => {
      if (gameState === 2) {
        switch (event.key) {
          ...
          case "ArrowLeft":
            this.changeActiveSkin(1, -1); // Move left for player one
            shortFartSound.play();
            break;
          case "ArrowRight":
            this.changeActiveSkin(1, 1); // Move right for player one
            shortFartSound.play();
            break;
          case "ArrowUp":
            this.changeActiveSkin(1, -3); // Move up for player one
            shortFartSound.play();
            break;
          case "ArrowDown":
            this.changeActiveSkin(1, 3); // Move down for player one
            shortFartSound.play();
            break;
          // Player Two character selection
          case "a":
            this.changeActiveSkin(0, -1); // Move left for player two
            shortFartSound.play();
            break;
          case "d":
            this.changeActiveSkin(0, 1); // Move right for player two
            shortFartSound.play();
            break;
          case "w":
            this.changeActiveSkin(0, -3); // Move up for player two
            shortFartSound.play();
            break;
          case "s":
            this.changeActiveSkin(0, 3); // Move down for player two
            shortFartSound.play();
            break;
        }
      }
